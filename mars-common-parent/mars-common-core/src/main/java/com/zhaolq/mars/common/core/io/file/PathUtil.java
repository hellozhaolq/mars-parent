...
import com.zhaolq.mars.common.core.io.file.visitor.MoveDirectoryVisitor;
...
public class PathUtil {
    ...
    public static boolean del(Path path) throws IORuntimeException {
        ...
        try {
            if (isDirectory(path)) {
                Files.walkFileTree(path, DeletingPathVisitor.withBigIntegerCounters());
            } else {
                delFile(path);
            }
        } catch (IOException e) {
    ...
    public static Path copyFile(Path src, Path target, CopyOption... options) throws IORuntimeException {
        ...
        Validate.notNull(target, "Destination File or directory is null !");
    ...
    public static Path copy(Path src, Path target, CopyOption... options) throws IORuntimeException {
        Validate.notNull(src, "Src path must be not null !");
        Validate.notNull(target, "Target path must be not null !");
    ...
    public static Path copyContent(Path src, Path target, CopyOption... options) throws IORuntimeException {
        ...
        try {
            Files.walkFileTree(src, new CopyDirectoryVisitor(Counters.bigIntegerPathCounters(), src, target, options));
        } catch (IOException e) {
            throw new IORuntimeException(e);
        }
        return target;
    ...
    public static Path moveContent(Path src, Path target, boolean isOverride) {
        ...
        try {
            ...
            try {
                Files.walkFileTree(src, new MoveDirectoryVisitor(Counters.bigIntegerPathCounters(), src, target, options));
                // 移动后空目录没有删除，
                del(src);
