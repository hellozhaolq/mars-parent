...
public class NetUtil {
    ...
    public static int getUsableLocalPort(int minPort, int maxPort) {
        ...
        String msg = String.format("Could not find an available port in the range [%d, %d] after %d attempts", minPort, maxPort, maxPort - minPort);
    ...
    public static TreeSet<Integer> getUsableLocalPorts(int numRequested, int minPort, int maxPort) {
        ...
        if (availablePorts.size() != numRequested) {
            String msg = String.format("Could not find %d available  ports in the range [%d, %d]", numRequested, minPort, maxPort);
            throw new UtilException(msg);
        }
    ...
    public static String toAbsoluteUrl(String absoluteBasePath, String relativePath) {
        try {
            ...
            String msg = String.format("To absolute url [%s] base [%s] error!", relativePath, absoluteBasePath);
    ...
    public static boolean isInRange(String ip, String cidr) {
        ...
        long cidrIpAddr = Ipv4Util.ipv4ToLong(cidr.substring(0, maskSplitMarkIndex));

        return (Ipv4Util.ipv4ToLong(ip) & mask) == (cidrIpAddr & mask);
    }
