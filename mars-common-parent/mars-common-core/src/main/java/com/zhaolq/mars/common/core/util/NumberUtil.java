...
public class NumberUtil {
     ...
     */
    private static final long[] FACTORIALS = new long[]{1L, 1L, 2L, 6L, 24L, 120L, 720L, 5040L, 40320L, 362880L, 3628800L, 39916800L, 479001600L,
    ...
    public static String decimalFormat(String pattern, double value) {
        Validate.isTrue(isValid(value), "value is NaN or Infinite!");
    ...
    public static String decimalFormat(String pattern, Object value, RoundingMode roundingMode) {
        if (value instanceof Number) {
            Validate.isTrue(isValidNumber((Number) value), "value is NaN or Infinite!");
        }
    ...
    public static int[] generateRandomNumber(int begin, int end, int size, int[] seed) {
        ...
        Validate.isTrue((end - begin) >= size, "Size is larger than range between begin and end!");
        Validate.isTrue(seed.length >= size, "Size is larger than seed size!");

        final int[] ranArr = new int[size];
        // 数量你可以自己定义。
        for (int i = 0; i < size; i++) {
            // 得到一个位置
            int j = RandomUtils.nextInt(0, seed.length - i);
            // 得到那个位置的数值
            ranArr[i] = seed[j];
            // 将最后一个未用的数字放到这里
    ...
    public static Integer[] generateBySet(int begin, int end, int size) {
        ...
        while (set.size() < size) {
            set.add(begin + RandomUtils.nextInt(0, end - begin));
        }
    ...
    public static BigInteger factorial(BigInteger start, BigInteger end) {
        Validate.notNull(start, "Factorial start must be not null!");
        Validate.notNull(end, "Factorial end must be not null!");
        if (start.compareTo(BigInteger.ZERO) < 0 || end.compareTo(BigInteger.ZERO) < 0) {
            throw new IllegalArgumentException(String.format("Factorial start and end both must be > 0, but got start=%d, end=%d", start, end));
        }
    ...
    public static long factorial(long n) {
        if (n < 0 || n > 20) {
            throw new IllegalArgumentException(String.format("Factorial must have n >= 0 and n <= 20 for n!, but got n = %d", n));
        }
    ...
    public static String toStr(Number number, boolean isStripTrailingZeros) {
        ...
        Validate.isTrue(isValidNumber(number), "Number is non-finite!");
        // 去掉小数点儿后多余的0
    ...
    public static String toStr(BigDecimal bigDecimal, boolean isStripTrailingZeros) {
        Validate.notNull(bigDecimal, "BigDecimal is null !");
        if (isStripTrailingZeros) {
